...
class HeliumAccountRewards(HttpStream):
    ...
    def __init__(self, account: str, start_date: datetime, **kwargs) -> None:
        super().__init__()
    ...
    def path(
        ...
        return f"{self.account}/rewards/sum"
...
class SourceHeliumAccountRewards(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        auth = NoAuth()
        start_date = datetime.strptime(config['date'], "%Y-%m-%d")
        return [HeliumAccountRewards(authenticator=auth,account=config['account'], start_date=start_date)]
